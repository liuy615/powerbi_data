...
class DataCenter:
    ...
    def get_apis(time_option: int) -> Dict[str, Dict[str, Any]]:
        ...
        # API配置模板
        api_templates = {
            '车辆销售明细表_订单日期': {'url': f'{Config.API_BASE_URL}/api/Summary/CarSalDetailSummary','method': 'GET','payload': {"TimeType": 99,"Type": 2,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '车辆销售明细表_开票日期': {'url': f'{Config.API_BASE_URL}/api/Summary/CarSalDetailSummary','method': 'GET','payload': {"TimeType": 99,"Type": "2","PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '车辆成本管理': {'url': f'{Config.API_BASE_URL}/api/Summary/CarCost','method': 'GET','payload': {"OrgType": "1","CostType": "1","TimeType": 99,"check": "2","PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '装饰_订单管理_装饰订单': {'url': f'{Config.API_BASE_URL}/api/Summary/AccessoryOrder','method': 'GET','payload': {"LikeType": "1","TimeType": 99,"PageSize": 3000,"PageNumber": 1,**time_params}},
            '套餐销售列表': {'url': f'{Config.API_BASE_URL}/api/AfterSales/SaleServicePackagePage','method': 'GET','payload': {"TimeType": 99,"isShowPhone": "1","PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '销售_车辆销售_成交订单': {'url': f'{Config.API_BASE_URL}/api/Summary/DealOrder','method': 'GET','payload': {"DateType": 99,"LikeType": "3","PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '销售_车辆销售_作废订单': {'url': f'{Config.API_BASE_URL}/api/Summary/AbnormalOrder','method': 'GET','payload': {"SalesContant": "1","state": "0,2","AuthDistrub": "0","TimeType": 99,"yType": 2,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '开票维护': {'url': f'{Config.API_BASE_URL}/api/Summary/GetInvoicesList','method': 'GET','payload': {"QueryType": "2","TimeType": 99,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '保险业务': {'url': f'{Config.API_BASE_URL}/api/Home/InternalExport','method': 'GET','payload': {"Salescontant": "1","TimeType": 99,"LikeType": '3',"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '按揭业务': {'url': f'{Config.API_BASE_URL}/api/Summary/MortgageData','method': 'GET','payload': {"Salescontant": "1","IsMortgage": "","IsPledge": "","state": "12","DateType": 99,"LikeType": '',"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '评估管理_成交': {'url': f'{Config.API_BASE_URL}/api/Summary/GetUsedCarList','method': 'GET','payload': {"BillState": "3","TimeType": 99,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '评估管理_已入库': {'url': f'{Config.API_BASE_URL}/api/Summary/GetUsedCarList','method': 'GET','payload': {"BillState": "5","TimeType": 99,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '调车结算查询': {'url': f'{Config.API_BASE_URL}/api/Summary/GetShuntSettlementList','method': 'GET','payload': {"OrgType": "-1","OrganizeId": "","PayState": 2,"BillingState": "","CarState": "","Title": "","TimeType": 99,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '销售_衍生_订单查询': {'url': f'{Config.API_BASE_URL}/api/Summary/SalePreorder','method': 'GET','payload': {"TimeType": 99,"LikeType": "3","PageSize": 3000,"PageNumber": 1,**time_params}},
            '库存车辆已售': {'url': f'{Config.API_BASE_URL}/api/Summary/StoreCar','method': 'GET','payload': {"OrgType": "0","CarState": "2","TimeType": 99,"ShowPriceFlag": "1","PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '汇票管理': {'url': f'{Config.API_BASE_URL}/api/Summary/CarTicket','method': 'GET','payload': {'sortName': 'PledgeDate','sortOrder': 'desc','DateType': '99',"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '销售回访': {'url': f'{Config.API_BASE_URL}/api/Summary/GetFollowUpList','method': 'GET','payload': {'TimeType': '99','Unvisit': '',"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            '未售订单': {'url': f'{Config.API_BASE_URL}/api/Summary/UnSaleCar','method': 'GET','payload': {'DateType': '1',"timetype": "1","LikeType": "3","PageSize": Config.PAGE_SIZE,"PageNumber": 1}},
            '计划车辆': {'url': f'{Config.API_BASE_URL}/api/Summary/PurchaseOrder','method': 'GET','payload': {"OrderState": "1","PageSize": Config.PAGE_SIZE,"PageNumber": 1}},
            '库存车辆查询': {'url': f'{Config.API_BASE_URL}/api/Summary/StoreCar','method': 'GET','payload': {"OrgType": "0","TimeType": "3","ShowPriceFlag": "1","PageSize": Config.PAGE_SIZE,"PageNumber": 1}},
            '删除日志': {'url': f'{Config.API_BASE_URL}/api/Summary/GetRecords', 'method': 'GET','payload': {"TimeType": "2", "PageSize": Config.PAGE_SIZE, "PageNumber": 1}},
        }
...
class DataSaver:
    ...
    def build_upsert_query(self, table_name: str, data_list: List[Dict],fieldnames: List[str], table_schema: Dict[str, str]) -> Tuple[Optional[str], Optional[List[Tuple]]]:
        """
        构建UPSERT查询语句
    ...
    def save_full_update_data(self, api_name: str, data_list: List[Dict]) -> int:
        ...
        try:
            ...
            delete_count = self.db_operator.execute_query(delete_query)
            self.logger.log_info(f"{api_name} 已清空表数据")
    ...
    def build_insert_query(self, table_name: str, data_list: List[Dict],fieldnames: List[str], table_schema: Dict[str, str]) -> Tuple[Optional[str], Optional[List[Tuple]]]:
        """
        构建INSERT查询语句
    ...
    def process_delete_log_data(self, data_list: List[Dict]) -> List[Dict]:
        """
        处理删除日志数据，解析Contents字段
        ...
        for item in data_list:
            ...
            try:
                ...
                if contents and isinstance(contents, str):
                    ...
                    # 提取Logo_IdOrCode
                    logo_id_or_code = content_json.get('Logo_IdOrCode')
                    if logo_id_or_code:
                        processed_item['Logo_IdOrCode'] = str(logo_id_or_code)

                    # 提取Arr数组中的信息
                    arr = content_json.get('Arr', [])
                    if arr and len(arr) > 0:
                        first_item = arr[0]
                        processed_item['RecordID'] = first_item.get('ID')
                        processed_item['BillCode'] = first_item.get('BillCode')

                # 确保必要的字段存在
                if 'Logo_IdOrCode' not in processed_item:
                    processed_item['Logo_IdOrCode'] = None
                if 'RecordID' not in processed_item:
                    processed_item['RecordID'] = None
                if 'BillCode' not in processed_item:
                    processed_item['BillCode'] = None

            except (json.JSONDecodeError, KeyError, AttributeError) as e:
                print(f"解析Contents字段失败: {str(e)}，内容: {contents}")
                # 如果解析失败，设置默认值
                processed_item['Logo_IdOrCode'] = None
                processed_item['RecordID'] = None
                processed_item['BillCode'] = None
    ...
    def save_delete_log_data(self, api_name: str, data_list: List[Dict]) -> int:
        ...
        """
        # 处理数据，解析Contents字段
        processed_data = self.process_delete_log_data(data_list)

        table_name = 'delete_log'
        table_schema = self.get_table_schema(table_name)
        if not table_schema:
            self.last_error = f"无法获取表 {table_name} 的结构，无法保存数据"
            return 0
        ...
        try:
            # 展平数据
            flattened_data, fieldnames = flatten_data(processed_data)

            # 确保必须的字段存在，如果没有则添加默认值
            required_fields = ['Title', 'Type', 'IPAddress', 'Contents', 'WriteTime', 'UserName']

            # 检查必须字段是否存在，如果不存在则添加默认值
            for item in flattened_data:
                for field in required_fields:
                    if field not in item:
                        print(f"警告: {field} 字段在数据中不存在，使用默认值")
                        if field == 'Code':
                            item[field] = 0  # Code字段默认为0
                        elif field == 'Title':
                            item[field] = ''
                        elif field == 'Type':
                            item[field] = ''
                            ...
                            item[field] = '{}'
                        elif field == 'WriteTime':
                            item[field] = ''
                        elif field == 'UserName':
                            item[field] = ''

            # 添加额外的字段到fieldnames中（如果不存在）
            extra_fields = ['Logo_IdOrCode', 'RecordID', 'BillCode']
            ...
            # 确保必须的字段在valid_fields中
            for field in required_fields:
                if field not in valid_fields and field in table_schema:
            ...
            for item in flattened_data:
                ...
                for field in valid_fields:
                    ...
                    # 确保必须字段不为None
                    if converted_value is None and field in required_fields:
                        if field == 'Code':
                            converted_value = 0
                        elif field in ['Title', 'Type', 'IPAddress', 'Contents', 'WriteTime', 'UserName']:
                            converted_value = ''
            ...
            affected_rows = self.db_operator.execute_without_primary_key(
                table_name, params_list, valid_fields
            )
    ...
    def save_data(self, api_name: str, data_list: List[Dict]) -> int:
        ...
        # 判断是否需要进行全量更新
        if table_name in self.FULL_UPDATE_TABLES:
        ...
        try:
            ...
            # 将数据保存完成的信息写入日志
            self.logger.log_info(f"{api_name} 数据保存完成，影响 {affected_rows} 行")
            return affected_rows
        except Exception as e:
            error_msg = f"{api_name} 数据处理失败: {str(e)}"
            self.logger.log_error(error_msg)
