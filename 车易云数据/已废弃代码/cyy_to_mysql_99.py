...
class DataCenter:
    ...
    def get_apis(time_option: int) -> Dict[str, Dict[str, Any]]:
        ...
        api_templates = {
            ...
            '装饰_订单管理_装饰订单': {'url': f'{Config.API_BASE_URL}/api/Summary/AccessoryOrder','method': 'GET','payload': {"LikeType": "1","TimeType": 99,"PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
            ...
            '销售_衍生_订单查询': {'url': f'{Config.API_BASE_URL}/api/Summary/SalePreorder','method': 'GET','payload': {"TimeType": 99,"LikeType": "3","PageSize": Config.PAGE_SIZE,"PageNumber": 1,**time_params}},
...
class DataSaver:
    ...
    # 需要全量更新的表列表（先删除后插入）
    FULL_UPDATE_TABLES = ['planned_vehicles', 'unsold_orders', 'inventory_vehicle_query']
    ...
    def save_full_update_data(self, api_name: str, data_list: List[Dict]) -> int:
        ...
        try:
            ...
            print(f"{api_name} 全量更新完成，插入 {affected_rows} 行数据")
        ...
        try:
            while True:
                ...
                for attempt in range(max_retries):
                    print(f"{task_name} 第{page_number}页 - 重试第{attempt + 1}次")
                    try:
                        response = self.requester.request(
                            url=url,
                            method=method,
