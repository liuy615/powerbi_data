...
class update_dashboard:
    def __init__(self):
        ...
        try:
            self.engine = create_engine(
                fr"mysql+pymysql://{self.db_config['user']}:{self.db_config['password']}@{self.db_config['host']}:{self.db_config['port']}/{self.db_config['database']}?charset=utf8mb4",echo=False
            )
            logging.info("读取数据库引擎创建成功")
        except Exception as e:
            logging.error(f"读取数据库引擎创建失败: {str(e)}", exc_info=True)
            raise
        ...
        try:
            ...
            logging.error(f"输出数据库引擎创建失败: {str(e)}", exc_info=True)
            raise
    ...
    def _write_df_to_db(self, df: pd.DataFrame, table_name: str) -> bool:
        """将DataFrame写入数据库"""
        try:
    ...
    def concat_newold_Tuis_dashboad(self):
        ...
        # 切片后加.copy()避免视图警告
        df_tuis_cyy = self.df_tuis_cyy[['订单门店', '业务渠道', '销售人员', '订单日期', '车系', '车型', '外饰颜色', '配置', '主播人员', '客户来源', '客户名称', '客户电话','作废类型', '退订原因', '退定日期', '非退定核算']].copy()  # 关键修复：加copy
    ...
    def concat_newold_Books_dashboad(self):
        # 切片后加.copy()避免视图警告
        df_books_cyy = self.df_books_cyy[['车架号', '订单日期', '定单日期', '销售人员', '定金金额', '审批状态', '定单归属门店', '所属团队', '客户来源', '车系', '外饰颜色', '车型', '配置', '定单状态', '主播人员', '联系电话', '联系电话2', '指导价']].copy()  # 关键修复：加copy
        # 切片后加.copy()避免视图警告
    ...
    def run(self):
        ...
        table_mapping = {
            ...
            '销售1.csv': 'sales',
            '二手车1.csv': 'used_cars',
            '未售锁车.csv': 'unsold_locked',
            '销售薪资.csv': 'sales_salary',
            '市场费用.csv': 'marketing_expenses',
            '所有车系.csv': 'all_car_series',
            '辅助_销售顾问.csv': 'assistant_sales_consultant'
        }
        
        # 筛选需要的列并写入数据库
        df_sales_mysql = df_sales[["公司名称", "销售日期", "车架号", "车系", "所属团队", '客户来源', "销售人员", "主播人员", "车主姓名", "联系电话", "提货价", "返利合计", "购买方式", "金融类型", "金融性质", "金融毛利", "上牌毛利", "二手车返利金额", "置换服务费", "单车毛利", "类型"]].copy()
