...
class CyysDataProcessorApp:
    ...
    def run(self):
        ...
        try:
            ...
            df = raw_data["未售订单"]
            df.rename(columns={"客户": "客户姓名", "客户电话": "联系电话", "客户电话2": "联系电话2"}, inplace=True)
            df.to_csv("E:/powerbi_data/看板数据/dashboard/未售订单.csv")
            # 开票数据筛选
            ...
            # 最终整理和导出
            (df_salesAgg_combined, df_salesAgg_, df_jingpin_result) = self.data_processor.finalize_and_export(df_salesAgg1, df_dings, df_jingpin_result, df_Ers1, df_diao2, df_inventory, df_inventory1)
