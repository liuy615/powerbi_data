...
# 1. 忽略无效转义序列警告（SyntaxWarning）
warnings.filterwarnings('ignore', category=SyntaxWarning, message='.*invalid escape sequence.*')

# 2. 忽略 SettingWithCopyWarning
...
class CyysDataProcessor:
    ...
    USED_CAR_REBATE_PATH = r"/看板数据/cyy_old_data/二手车返利存档.csv"
    ...
    def _read_basic_data(self):
        ...
        try:
            ...
            logging.info(f"服务网络数据读取完成：{len(self.df_service_net)}条车系记录")
        except Exception as e:
            logging.error(f"服务网络数据读取失败：{str(e)}")
            self.df_service_net = pd.DataFrame()

        # 读取所有源库表数据
        self.raw_data = {
            df_name: self.read_from_mysql(table_name)
            for df_name, table_name in self.API_TABLE_MAPPING.items()
        }
    ...
    def _preprocess_field_mapping(self):
        ...
        for df_name, df_raw in self.raw_data.items():
            ...
            if df_name not in self.dfname_to_col_rename or df_raw.empty:
                logging.warning(f"跳过数据类型[{df_name}]：无映射或数据为空")
                continue
            ...
            if not valid_cols:
                logging.warning(f"数据类型[{df_name}]无有效字段，跳过处理")
                continue

            # 筛选并批量重命名（英文→中文）
            df_processed = df_raw[valid_cols].rename(columns=rename_map)
            self.processed_data[df_name] = df_processed
            logging.info(f"数据类型[{df_name}]处理完成：{len(valid_cols)}个字段，{len(df_processed)}条数据")
    ...
    def _clean_insurance(self, df_insurance):
        if df_insurance.empty:
            logging.warning("新车保险数据为空")
            return pd.DataFrame()

        logging.info(f"新车保险数据清洗：{len(df_insurance)}条记录")
        return df_insurance

    """清洗二手车成交与入库数据"""
    def _clean_used_car(self, df_ershou, df_ershou1):
        df_Ers = pd.concat([df_ershou, df_ershou1], axis=0, ignore_index=True)
        df_Ers = df_Ers[df_Ers['收款状态'] == '已收款']  # 只保留已收款
        logging.info(f"二手车数据清洗完成：{len(df_Ers)}条已收款记录")
        return df_Ers
    ...
    def _clean_decoration(self, df_decoration):
        ...
        if df_decoration.empty or '销售合计' not in df_decoration.columns:
            logging.warning("装饰订单数据不完整，跳过清洗")
            return df_decoration_final, df_jingpin_final

        # 数据类型转换
        df_decoration = df_decoration.dropna(subset=['开票日期', '收款日期'], how='all')
        ...
        # 赠送装饰筛选
        cond_gift = (df_decoration['单据类型'] == '新车销售') | (df_decoration['单据类型'].isin(['客户增购', '换货销售', '独立销售']) & (df_decoration['销售合计'] == 0))
        ...
        if '车架号' in df_decoration_gift.columns:
            ...
            if all(col in df_decoration_gift.columns for col in cost_cols):
                ...
                df_decoration_final = df_decoration_gift.groupby('车架号')[['装饰赠送成本', '销售合计']].sum().reset_index().merge(gift_items, on='车架号', how='left')
                df_decoration_final.rename(columns={'装饰赠送成本': '装饰成本','销售合计': '装饰收入','物资名称': '赠送装饰项目'}, inplace=True)
        # 精品销售筛选
        ...
        if all(col in df_jingpin.columns for col in ['订单编号', '物资名称', '出/退/销数量']):
            ...
            jingpin_items = df_jingpin.groupby('订单编号').apply(
                lambda x: ','.join(f"{name}*{qty}" for name, qty in zip(x['物资名称'], x['出/退/销数量'])
                                   if pd.notna(name) and pd.notna(qty)),include_groups=False).reset_index(name='物资明细')
            ...
            if all(col in df_jingpin.columns for col in cost_cols):
                ...
                group_cols = ['订单门店', '订单编号', '车架号', '开票日期', '收款日期', '销售顾问', '客户名称', '联系电话']
                valid_group_cols = self._get_valid_columns(df_jingpin, group_cols)

                df_jingpin_final = df_jingpin.groupby(valid_group_cols)[['总成本', '销售合计', '出/退/销数量']].sum().reset_index().merge(jingpin_items, on='订单编号', how='left')
                df_jingpin_final.rename(columns={'销售合计': '销售总金额','销售顾问': '精品销售人员','出/退/销数量': '总次数'}, inplace=True)
                df_jingpin_final['毛利润'] = df_jingpin_final['销售总金额'] - df_jingpin_final['总成本']

        logging.info(f"装饰数据清洗完成：赠送装饰{len(df_decoration_final)}条，精品销售{len(df_jingpin_final)}条")
        return df_decoration_final, df_jingpin_final
    ...
    def _clean_planned_vehicles(self, df_plan, df_debit):
        ...
        logging.info(f"计划车辆数据清洗完成：{len(df_plan)}条记录")
        return df_plan
    ...
    """清洗订单数据"""
    def _clean_orders(self, df_books, df_books2, tui_dings_df, df_unsold):
        if df_books.empty:
        ...
        if not df_books2.empty:
            ...
            df_books2 = df_books2[['ID', '车架号', '联系电话', '联系电话2', '主播人员', '车系', '客户姓名', '订单公司']].drop_duplicates()
        # 处理未售订单,获取主播人员
        ...
        df_zhubolist[cols] = (
            df_zhubolist[cols]
            .replace('', pd.NA)  # 空字符串也当成缺失
            .fillna(0)  # 缺失补 0
            .astype('int64')  # 此时已无 ''，不会报错
            .astype('str')
            .replace('0', '')  # 把补位的 0 再变回空串
        )
        ...
        if '联系电话2' in df_dings.columns:
            df_dings['联系电话2'] = df_dings['联系电话2'].astype(str)
    ...
    def _clean_canceled_orders(self, tui_dings_df):
        ...
        # 筛选必要字段
        cancel_cols = ['订单门店', '业务渠道', '销售人员', '订单日期', '车系', '外饰颜色','车型', '配置', '主播人员', '客户名称', '客户电话', '作废类型', '退订原因', '退定日期', '非退定核算']
    ...
    def _clean_sales_invoice(self, df_salesAgg):
        ...
        df_salesAgg_clean.rename(columns={
            '入库日期': '到库日期',
            '订单门店': '匹配定单归属门店',
            '订单日期': '定单日期',
            '开票日期': '销售日期',
        ...
        df_salesAgg_clean = df_salesAgg_clean[['车架号', '车系', '车型', '车辆配置', '外饰颜色', '定金金额', '指导价', '提货价', '匹配定单归属门店',
        ...
        if '所属团队' in df_salesAgg_clean.columns:
            df_salesAgg_clean = df_salesAgg_clean[(df_salesAgg_clean['所属团队'] != "调拨") &(df_salesAgg_clean['所属团队'].notna())]

        logging.info(f"销售明细（开票日期）清洗完成：{len(df_salesAgg_clean)}条记录")
    ...
    def _process_invoice_data(self, df_kaipiao):
        if df_kaipiao.empty:
            logging.warning("开票维护数据为空，跳过筛选")
            return pd.DataFrame()
        df_kaipiao = df_kaipiao[df_kaipiao['单据类别'] == "车辆销售单"][['车架号', '开票门店']].drop_duplicates(subset='车架号', keep='last')
        logging.info(f"车辆销售开票数据筛选完成：{len(df_kaipiao)}条记录")
        return df_kaipiao
    ...
    def _process_used_car_rebate(self, df_Ers, df_kaipiao):
        ...
        if not df_Ers.empty:
            try:
                ...
                df_ers_rebate2.rename(columns={'车架号': '置换车架号_车牌','置换车架号': '车架号','其他费用': '二手车返利金额1'}, inplace=True)
                logging.info(f"二手车返利处理完成：无置换{len(df_ers_rebate1)}条，有置换{len(df_ers_rebate2)}条")
            except Exception as e:
    ...
    def _create_sales_main_data(self, df_salesAgg):
        ...
        sales_cols = [
            ...
            '代金券', '金融押金', '保险押金', '其它押金', '其它费用', '特殊事项', '拖车费用'
        ]
    ...
    """计算销售毛利数据（多表合并与衍生计算）"""
    def _calculate_profit(self, df_sales_main, df_books2, df_service_final, df_carcost, df_loan, df_decoration_final, df_kaipiao, df_ers_rebate2):
        if df_sales_main.empty:
            logging.warning("销售主数据为空，无法进行毛利计算")
            return pd.DataFrame()
        ...
        # 合并主播信息
        if not df_books2.empty and all(col in df_books2.columns for col in ['车架号', '联系电话', '主播人员']):
            df_sales_profit = pd.merge(df_sales_profit, df_books2[['车架号', '联系电话', '主播人员']],on='车架号', how='left')

        # 合并套餐数据
        if not df_service_final.empty:
            df_sales_profit = df_sales_profit.merge(df_service_final[['车架号', '保养升级成本', '套餐明细']],on='车架号', how='left')
        ...
        if not df_decoration_final.empty:
            df_sales_profit = df_sales_profit.merge(df_decoration_final[['车架号', '装饰成本', '装饰收入', '赠送装饰项目']],on='车架号', how='left')
        ...
        if not df_ers_rebate2.empty:
            df_sales_profit = df_sales_profit.merge(df_ers_rebate2[['车架号', '二手车返利金额1', '收款日期']],on='车架号', how='left')

        # 合并二手车返利存档
        try:
            df_ers_rebate_archive = pd.read_csv(self.USED_CAR_REBATE_PATH)
            df_ers_rebate_archive = df_ers_rebate_archive[df_ers_rebate_archive["销售日期"] > "2025-05-01"]
            if not df_ers_rebate_archive.empty:
                df_sales_profit = df_sales_profit.merge(df_ers_rebate_archive[['车架号', '二手车返利金额']],on='车架号', how='left')
        except Exception as e:
            logging.warning(f"未合并二手车返利存档：{str(e)}")

        logging.info(f"销售毛利主数据合并完成：{len(df_sales_profit)}条记录")

        # 金额字段转换为float
        float_cols_profit = [
            '定金金额', '指导价', '裸车成交价', '车款（发票价）', '提货价', '调拨费', '上牌费',
            '置换款', '精品款', '代金券', '其它押金', '其它费用', '金融押金', '保险押金',
            '置换服务费', '金融服务费_顾问', '票据事务费', '调整项', '特殊事项', '拖车费用']
        ...
        if all(col in df_sales_profit.columns for col in ['代金券', '金融押金', '保险押金', '其它押金']):
            df_sales_profit['后返客户款项'] = df_sales_profit[['代金券', '金融押金', '保险押金', '其它押金']].sum(axis=1)
        else:
            df_sales_profit['后返客户款项'] = 0

        # 二手车返利金额合并
        if '二手车返利金额' in df_sales_profit.columns and '二手车返利金额1' in df_sales_profit.columns:
            df_sales_profit['二手车返利金额'] = df_sales_profit['二手车返利金额'].fillna(df_sales_profit['二手车返利金额1'])
        ...
        if all(col in df_sales_profit.columns for col in ['车款（发票价）', '置换款', '后返客户款项', '精品款']):
            df_sales_profit['销售车价'] = df_sales_profit[['车款（发票价）', '置换款', '后返客户款项']].sum(axis=1) - df_sales_profit['精品款']

        # 贵州区域促销费用（后返+200）
        if '公司名称' in df_sales_profit.columns and '所属团队' in df_sales_profit.columns:
            df_sales_profit['促销费用'] = np.where(
                (df_sales_profit['公司名称'].isin(self.GUIZHOU_COMPANIES)) & (df_sales_profit['所属团队'] != "调拨"),
        ...
        if all(col in df_sales_profit.columns for col in
            ...
            try:
                ...
                # 生成车系辅助列（处理空值避免拼接异常）
                df_sales_profit['车系辅助'] = (df_sales_profit['车系'].fillna('') +df_sales_profit['车型'].fillna(''))
                # 转换销售日期为datetime类型（兼容异常日期格式）
                df_sales_profit['销售日期'] = pd.to_datetime(df_sales_profit['销售日期'], errors='coerce')


                # 获取有效车系辅助列表
                df_vat['起始日期'] = df_vat['起始日期'].apply(lambda x: x if pd.isna(x) else str(x))
                df_vat['起始日期'] = pd.to_datetime(df_vat['起始日期'], errors='coerce')
            except Exception as e:
                logging.error(f"汉唐增值税数据处理失败: {str(e)}，将使用默认税费计算逻辑")
                df_vat_list = []  # 空列表确保后续条件判断不生效

            # 合并增值税相关信息
            df_sales_profit = pd.merge(df_sales_profit, df_vat[['辅助列', '抵扣金额', '起始日期', '最终结算价（已抵扣超级置换）']], left_on='车系辅助',right_on='辅助列', how='left')
            df_sales_profit['抵扣金额'] = df_sales_profit['抵扣金额'].fillna(0)
            df_sales_profit['最终结算价（已抵扣超级置换）'] = pd.to_numeric(df_sales_profit['最终结算价（已抵扣超级置换）']).fillna(0)
            df_sales_profit['起始日期'] = df_sales_profit['起始日期'].fillna(pd.Timestamp('1900-01-01'))

            # 新税费计算逻辑
            condition = (df_sales_profit['销售日期'] >= df_sales_profit['起始日期']) & (df_sales_profit['辅助列']==df_sales_profit['车系辅助']) & (df_sales_profit['提货价']<=df_sales_profit['最终结算价（已抵扣超级置换）']) & (df_sales_profit['置换款']>0)

            df_sales_profit['税费'] = np.where(
                condition,
                # 符合条件：利润差减10000后再计算税费（仅正数部分）
                np.where(
                    df_sales_profit['增值税利润差'] - df_sales_profit["抵扣金额"] > 0,
                    np.round((df_sales_profit['增值税利润差'] - df_sales_profit["抵扣金额"]) / 1.13 * 0.13 * 1.12, 2),
                    0),
                # 不符合条件：使用原有计算逻辑
                np.where(
        ...
        if all(col in df_sales_profit.columns for col in ['销售车价', '返利合计', '税费', '提货价']):
            df_sales_profit['毛利'] = df_sales_profit[['销售车价', '返利合计']].sum(axis=1) - df_sales_profit[['税费', '提货价']].sum(axis=1)
        else:
            df_sales_profit['毛利'] = 0

        logging.info("销售毛利基础计算完成")
    ...
    def _process_transfer_data(self, df_diao, df_sales_profit):
        ...
        if '调拨费' not in df_transfer.columns:
            logging.warning("调拨数据缺少调拨费字段，无法处理")
            return pd.DataFrame()
        ...
        # 合并销售数据补充字段
        if not df_sales_profit.empty and all(col in df_sales_profit.columns for col in ['车架号', '销售日期', '车系', '车型', '车辆配置', '调拨费']):
            df_transfer_merge = pd.merge(df_transfer, df_sales_profit[['车架号', '销售日期', '车系', '车型', '车辆配置', '调拨费']],on='车架号', how='left')
        ...
        df_transfer_merge.rename(columns={
            '车系': '车系1',
            '调出门店': '公司名称',
            '支付门店': '车主姓名'
        }, inplace=True)
        ...
        df_transfer_final = df_transfer_final[(df_transfer_final['调拨费']!=0) & (df_transfer_final['调拨费'].notnull())]
        logging.info(f"调拨数据处理完成：{len(df_transfer_final)}条有效记录")
    ...
    def _finalize_profit_data(self, df_sales_profit, df_ers_rebate1, df_transfer_final):
        ...
        if not df_sales_profit.empty:
            ...
            df_sales_profit['单车毛利'] = df_sales_profit[valid_profit_cols].sum(axis=1) - df_sales_profit[valid_deduct_cols].sum(axis=1)
        ...
        if not df_ers_rebate1.empty:
            df_ers_rebate1 = df_ers_rebate1[df_ers_rebate1["收款日期"] > "2025-04-01"]
            dfs_to_combine.append(df_ers_rebate1)
        if not df_transfer_final.empty:
            dfs_to_combine.append(df_transfer_final)
        df_profit_combined = pd.concat(dfs_to_combine, axis=0, ignore_index=True, join='outer')
        ...
        logging.info(f"综合毛利数据合并完成：共{len(df_profit_combined)}条记录")
        return df_profit_combined

    """写入mongdb的数据清洗"""
    def clean_data_mongdb(self, sales_data, df_jingpin_result):
        # 1. 销售毛利数据
        df_salesAgg = sales_data.copy()
        df_salesAgg.rename(columns={
            '公司名称': '订单门店',
            '订车日期': '订车日期',
            '销售日期': '开票日期',
        ...
        df_salesAgg = df_salesAgg[
            ['车源门店', '供应商', '订单门店', '订车日期', '开票日期', '收款日期', '车架号', '发动机号', '车辆车系',
             '车辆车型','车辆颜色', '业务渠道', '销售人员', '邀约人员', '交付专员', '主播人员', '客户名称', '身份证号', '联系电话',
             '联系电话2', '订金金额','厂家官价', '裸车成交价', '销售车价', '开票价格', '最终结算价',
             ...
             '上牌服务费', '上牌毛利', '二手车成交价', '二手车返利', '置换服务费', '促销费用', '赠送装饰项目',
             '装饰收入','装饰成本', '套餐明细', '保养升级成本', '装饰赠送合计', '其他成本', '返介绍费', '合作返利',
             '综合结算服务费', '调拨费', '票据事务费', '票据事务费-公司', '其它费用', '特殊事项', '拖车费用', '区补',
             '质损赔付金额', '调整项', '单车毛利',
        ...
        float_columns = ['订金金额', '厂家官价', '裸车成交价', '销售车价', '开票价格', '最终结算价',
                         ...
                         '增值税利润差', '税费',
                         '毛利', '返介绍费', '区补', '退代金券', '退成交车辆定金（未抵扣）', '退按揭押金',
                         '退置换补贴保证金', '质损赔付金额',
                         '首付金额', '贷款总额', '实收金融服务费', '厂家贴息', '公司贴息', '返利金额', '金融税费',
                         '金融毛利', '上牌费', '上牌服务费', '上牌毛利',
                         '二手车返利', '置换服务费', '促销费用', '保养升级成本', '装饰成本', '装饰赠送合计', '其他成本',
                         '合作返利', '综合结算服务费', '调拨费', '票据事务费', '票据事务费-公司', '单车毛利',
                         '二手车成交价', '装饰收入', '调整项', '其它费用', '特殊事项', '拖车费用']

        # 定义需要转换为字符串类型的列
        string_columns = ['车源门店', '供应商', '订单门店', '订车日期', '开票日期', '收款日期', '车架号', '发动机号',
                          '车辆车系', '车辆车型', '车辆颜色', '业务渠道',
                          '销售人员', '客户名称', '身份证号', '联系电话', '联系电话2', '金融类型', '购买方式',
                          '按揭渠道', '期限', '按揭产品', '赠送装饰项目', '返利系数', '套餐明细', '开票门店',
                          '调出类型', '邀约人员', '交付专员', '主播人员']

        # 执行数据类型转换
        try:
            df_salesAgg[string_columns] = df_salesAgg[string_columns].replace('nan', '').fillna('').astype('str')
        except Exception as e:
            logging.error(f"字符串列类型转换出错: {str(e)}")

        try:
            df_salesAgg[float_columns] = df_salesAgg[float_columns].apply(pd.to_numeric, errors='coerce').fillna(
                0).astype('str')
        except Exception as e:
            logging.error(f"数值列类型转换出错: {str(e)}")

            # 转换日期列
            date_columns = ['开票日期', '收款日期']
            for col in date_columns:
                if col in df_jingpin_result.columns:
                    df_jingpin_result[col] = pd.to_datetime(df_jingpin_result[col], format='mixed', errors='coerce')
            # 设置过滤条件
            start_date = datetime(2025, 4, 1)

            # 先处理 df_jingpin_result 的日期
            filtered_df_jingpin_result = df_jingpin_result[df_jingpin_result['收款日期'] >= start_date]
            filtered_df_jingpin_result.loc[:, '收款日期'] = filtered_df_jingpin_result['收款日期'].apply(
                lambda x: x.strftime('%Y/%m/%d') if pd.notna(x) else None
            )
            filtered_df_jingpin_result = filtered_df_jingpin_result.drop_duplicates()
            filtered_df_jingpin_result = filtered_df_jingpin_result.astype('str')
            filtered_df_jingpin_result['订单门店'] = np.where(
                filtered_df_jingpin_result['订单门店'].str.contains('直播基地'),
                '直播基地',
                filtered_df_jingpin_result['订单门店']
            )

            # 处理 df_salesAgg4 的日期列，在进行筛选前保持为 datetime 类型
            ...
            filtered_df = filtered_df[
                ['订单门店', '订车日期', '开票日期', '车架号', '车辆车系', '车辆车型', '车辆颜色', '业务渠道',
                 '销售人员', '邀约人员', '交付专员', '客户名称', '身份证号',
                 '联系电话', '联系电话2', '订金金额', '厂家官价', '裸车成交价', '销售车价', '开票价格', '最终结算价',
                 '置换补贴保证金', '票据事务金额', '后返客户款项', '保险返利',
                 '终端返利', '厂家返利合计', '增值税利润差', '税费', '毛利', '上牌费', '上牌服务费', '上牌毛利',
                 '质损赔付金额', '单车毛利', '开票门店']]

        df_salesAgg.to_csv("df_salesAgg.csv")
        filtered_df_jingpin_result.to_csv("filtered_df_jingpin_result.csv")
    ...
    def _add_primary_key(self, table_name, primary_keys):
        ...
        try:
            ...
            if missing_cols:
                logging.warning(f"表[{table_name}]缺少主键字段{missing_cols}，跳过主键添加")
                return

            # 检查是否已有主键
            constraints = insp.get_pk_constraint(table_name)
            if constraints['constrained_columns']:
                logging.info(f"表[{table_name}]已存在主键：{constraints['constrained_columns']}")
                return
            ...
            with self.output_engine.connect() as conn:
                ...
                conn.commit()
                logging.info(f"表[{table_name}]添加主键：{primary_keys}")
        except SQLAlchemyError as e:
            logging.error(f"表[{table_name}]添加主键失败：{str(e)}")
    ...
    def _write_all_tables(self, df_profit_combined, df_dings, df_inventory_all, tui_dings_df, df_debit, df_salesAgg_clean, df_jingpin_final, df_inventory1):
        logging.info(f"开始写入输出数据库[{self.OUTPUT_MYSQL_CONFIG['database']}]")
    ...
    def export_to_mongodb(self, sales_data, jingpin_data, diaobo_data):
        ...
        try:
            # 连接到 MongoDB 数据库
            client = MongoClient(
                'mongodb://xg_wd:H91NgHzkvRiKygTe4X4ASw@192.168.1.7:27017/xg?authSource=xg&authMechanism=SCRAM-SHA-256')
            db = client['xg']
    ...
    def run(self):
        ...
        df_salesAgg = df_salesAgg[df_salesAgg["开票日期"] > '2025-04-01']
        ...
        df_unsold.to_csv("E:/powerbi_data/看板数据/cyy_old_data/未售订单.csv")
        ...
        df_dings = self._clean_orders(df_books, df_books2, tui_dings_df, df_unsold)  # 订单数据清洗
        ...
        logging.info("数据清洗阶段完成")

        # 4. 毛利计算与数据合并
        logging.info("开始计算综合毛利数据")
        ...
        # 5. 写入输出数据库
        self._write_all_tables(df_profit_combined, df_dings, df_inventory_all,tui_dings_df_clean, df_debit_clean, df_salesAgg_clean,df_jingpin_final, df_inventory1_clean)

        # 6. 上传数据到mogodb
        # self.clean_data_mongdb(df_profit_combined, df_jingpin_final)
        ...
        logging.info("=" * 50)
        logging.info("车易云商数据处理流程全部完成！")
        logging.info(f"输出数据库：{self.OUTPUT_MYSQL_CONFIG['database']}")
        logging.info("=" * 50)
